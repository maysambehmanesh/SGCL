...
class DualBranchContrast(torch.nn.Module):
    ...
    def forward(self, h1=None, h2=None, g1=None, g2=None, batch=None, h3=None, h4=None, L1=None, L2=None, edge_index1=None, edge_index2=None,
        ...
        l1 = self.loss(anchor=anchor1, sample=sample1, pos_mask=pos_mask1, neg_mask=neg_mask1, L=L1, edge_index=edge_index1, **self.kwargs)
        l2 = self.loss(anchor=anchor2, sample=sample2, pos_mask=pos_mask2, neg_mask=neg_mask2, L=L2, edge_index=edge_index2, **self.kwargs)
...
class WithinEmbedContrast(torch.nn.Module):
    ...
    def forward(self, h1, h2, L1=None, L2=None, edge_index1=None, edge_index2=None, s_type=None, s_param=None):
        if self.mode == 'L2L':
            assert h1 is not None and h2 is not None
            anchor1, sample1, pos_mask1, _ = self.sampler(anchor=h1, sample=h2)
            anchor2, sample2, pos_mask2, _ = self.sampler(anchor=h2, sample=h1)
            
        l1 = self.loss(anchor=h1, sample=h2, pos_mask=pos_mask1, L=L1, edge_index=edge_index1, s_type=s_type, s_param=s_param, **self.kwargs)
        l2 = self.loss(anchor=h2, sample=h1, pos_mask=pos_mask2, L=L2, edge_index=edge_index2, s_type=s_type, s_param=s_param, **self.kwargs)
        return (l1 + l2) * 0.5



class BothContrast(torch.nn.Module):
    def __init__(self, loss_LL: Loss, loss_GL: Loss, intraview_negs: bool = False, **kwargs):
        super(BothContrast, self).__init__()
        self.loss_LL = loss_LL
        self.loss_GL = loss_GL
        self.sampler_LL = get_sampler('L2L', intraview_negs=intraview_negs)
        self.sampler_GL = get_sampler('G2L', intraview_negs=intraview_negs)
        self.kwargs = kwargs
    ...
    def forward(self, h1=None, h2=None, g1=None, g2=None, batch=None, h3=None, h4=None, L1=None, L2=None, edge_index1=None, edge_index2=None,
        ...
        ## 'L2L'
        assert h1 is not None and h2 is not None
        anchor1, sample1, pos_mask1, neg_mask1 = self.sampler_LL(anchor=h1, sample=h2)
        anchor2, sample2, pos_mask2, neg_mask2 = self.sampler_LL(anchor=h2, sample=h1)
        
        pos_mask1, neg_mask1 = add_extra_mask(pos_mask1, neg_mask1, extra_pos_mask, extra_neg_mask)
        pos_mask2, neg_mask2 = add_extra_mask(pos_mask2, neg_mask2, extra_pos_mask, extra_neg_mask)
        l1 = self.loss_LL(anchor=anchor1, sample=sample1, pos_mask=pos_mask1, neg_mask=neg_mask1, L=L1, edge_index=edge_index1, **self.kwargs)
        l2 = self.loss_LL(anchor=anchor2, sample=sample2, pos_mask=pos_mask2, neg_mask=neg_mask2, L=L2, edge_index=edge_index2,**self.kwargs)
        
        ## 'G2L'        
        if batch is None or batch.max().item() + 1 <= 1:  # single graph
            assert all(v is not None for v in [h1, h2, g1, g2, h3, h4])
            anchor1, sample1, pos_mask1, neg_mask1 = self.sampler_GL(anchor=g1, sample=h2, neg_sample=h4)
            anchor2, sample2, pos_mask2, neg_mask2 = self.sampler_GL(anchor=g2, sample=h1, neg_sample=h3)
        else:  # multiple graphs
            assert all(v is not None for v in [h1, h2, g1, g2, batch])
            anchor1, sample1, pos_mask1, neg_mask1 = self.sampler_GL(anchor=g1, sample=h2, batch=batch)
            anchor2, sample2, pos_mask2, neg_mask2 = self.sampler_GL(anchor=g2, sample=h1, batch=batch)

        pos_mask1, neg_mask1 = add_extra_mask(pos_mask1, neg_mask1, extra_pos_mask, extra_neg_mask)
        pos_mask2, neg_mask2 = add_extra_mask(pos_mask2, neg_mask2, extra_pos_mask, extra_neg_mask)
        l3 = self.loss_GL(anchor=anchor1, sample=sample1, pos_mask=pos_mask1, neg_mask=neg_mask1, L=L1, **self.kwargs)
        l4 = self.loss_GL(anchor=anchor2, sample=sample2, pos_mask=pos_mask2, neg_mask=neg_mask2, L=L2, **self.kwargs)

        return (l1 + l2 + l3 + l4) * 0.25